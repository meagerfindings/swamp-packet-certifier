...
| `invocationId`    | `string`    | yes      | —       | Effective invocation identifier bound to the certification report.                                                                                                                                                                                       |
| `snapshotInvocationId` | `string`    | no       | `invocationId` | Original pre-agent snapshot identity. Use this for a fallback invocation so certification retains the initial baseline.                                                                                                                                 |
| `allowedPaths`    | `string[]`  | yes      | —       | Exact repository-relative paths the packet may change.                                                                                                                                                                                          |
| `maxChangedFiles` | `number`    | no       | `3`     | Maximum changed tracked and untracked files, up to 25.                                                                                                                                                                                          |
